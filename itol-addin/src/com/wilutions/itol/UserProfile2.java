...
public class UserProfile2 {
	 ...
	 *            Application name
	 */
	public UserProfile2(String manufacturerName, String appName) {
	...
	private void load() {
		...
		try {
			...
			this.root = new JSONObject(new String(bytes, "UTF-8"));
		}
		catch (Exception e) {
			this.root = new JSONObject();
		}
	...
	private void RegUtil_deleteRegistryValue(String key, String fieldName) {
	...
	private Object RegUtil_getRegistryValue(String keyName, String valueName, Object defaultValue) {
		...
		if (root.has(keyName)) {
			child = root.getJSONObject(keyName);
			if (child.has(valueName)) {
				ret = child.getString(valueName);
			}
		}
		return ret;
	}
	...
	private void RegUtil_setRegistryValue(String keyName, String valueName, Object value) {
		...
		if (root.has(keyName)) {
			...
			root.put(keyName, child);
		}
		child.put(valueName, value);
	}

	public static void main(String[] args) {
		UserProfile2 registry = new UserProfile2("WILUTIONS", "TEST-REGISTRY");
